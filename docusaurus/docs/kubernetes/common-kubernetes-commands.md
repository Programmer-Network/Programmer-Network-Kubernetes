...
16. **Launch a debug pod for troubleshooting (basic busybox container in interactive terminal):**
```bash
kubectl run debug --image=busybox -it --rm -- /bin/sh
```

17. **Forcefully delete a pod (if stuck in terminating or other strange states):**
```bash
...
20. **Forward a local port to a pod (e.g., for local access to service, like database):**
```bash
...
32. **List PostgreSQL-related resources (assuming you have CRDs or a PostgreSQL operator installed):**
```bash
...
35. **View resource usage (CPU/Memory) for nodes and pods (requires metrics-server):**
- **For nodes:** 
    ```bash
    kubectl top nodes
    ```
...
43. **Get the history of resource changes for a deployment (e.g., when scaling happens):**
```bash
