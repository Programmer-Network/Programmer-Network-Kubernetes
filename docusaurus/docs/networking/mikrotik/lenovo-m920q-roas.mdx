...
If everything went well up until this point, it means we are ready to do the final step, connect to our new amazing router and purchase the license.

Unless you are about to register a mini ISP company and sell internet, just like myself, you'll want to get RouterOS Level 4 license. You can compare the licenses under [RouterOS license key levels](https://help.mikrotik.com/docs/spaces/ROS/pages/328149/RouterOS+license+keys#RouterOSlicensekeys-RouterOSlicensekeylevels). In short, you'll notice that the main difference between license level 4 and higher ups, are things like OVPN, Tunnels, EoIP, User Management, RADIUS, etc. In simple words, we don't care about the majority of those.

To purchase the license, you will need to create a MikroTik account at [MikroTik Account](https://mikrotik.com/client/) page. Once your account is created, you will get your initial login information via email. (Make sure to change the password once you log in).

Once you purchase the license, you will get an email with a subject `MikroTik RouterOS Licensed Key`, which will contain your license in a `.key` file. You will download the license, login to your router via e.g. winbox, go to `System > License` and click `Import Key...`. Once you import the key, your router will prompt you that it has to be restarted and after it does so, your new level 4 license will be applied to your device.
